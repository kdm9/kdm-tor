...
node_addrs_changed(node_t *node)
{
  if (node->last_reachable) {
    SMARTLIST_FOREACH(node->last_reachable, addr_reachability_t *, ar,
                      tor_free(ar));
...
/** Find the time we last knew all addresses of family <b>af</b> to be
 * reachable. If we don't know anything about this address family, return 0. */
time_t
